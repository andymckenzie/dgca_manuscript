...
# harcode the parameters during a test
if(test){
	n_samples = 30
	seed = 50 #42

	ddcor_test = TRUE
	EBcoexpress_test = TRUE
	discordant_test = TRUE
	compare_ddcor_eb_discordant = TRUE

	corr_classes = "medium" #"all" "medium" "strong"

	compare_ddcor_classes = FALSE
	compare_eb_classes = FALSE
	compare_discordant_classes = FALSE
}
...
if(!test){
	...
	corr_classes = as.character(args[7]) #"all" "medium" "strong"

	compare_ddcor_classes = as.logical(args[8])
	compare_eb_classes = as.logical(args[9])
	compare_discordant_classes = as.logical(args[10])
}
...
###########################################
# quality control of the resulting simulation data

test_for_unintended_correlations = FALSE
if(test_for_unintended_correlations){
	cor_sims_a_pearson = cor(sim_data_a)
	cor_sims_a_rem_pearson = subsup_diag(cor_sims_a_pearson, 0)
	diag(cor_sims_a_rem_pearson) = 0
	corrplot(cor_sims_a_rem_pearson)

	cor_sims_a_spearman = cor(sim_data_a, method = "spearman")
	cor_sims_a_rem_spearman = subsup_diag(cor_sims_a_spearman, 0)
	diag(cor_sims_a_rem_spearman) = 0
	corrplot(cor_sims_a_rem_spearman)
}
...
ddcor_line = c("ddcor", corr_classes, n_samples, seed, ddcor_time, as.numeric(full_roc$auc))
write.table(ddcor_line, "auc_results_classes.txt", sep = "\t", quote = FALSE, append = TRUE, col.names = FALSE,
	row.names = FALSE)
...
if(compare_ddcor_classes){
	...
	tiff(file = paste0("ddcor_classes_", corr_classes, n_samples, "_samples", seed, ".tiff"),
...
print(paste0("time taken to run the EBcoexpress pipeline with ", corr_classes, n_samples, " samples..."))
...
ddcor_line = c("ebcoexpress", corr_classes, n_samples, seed, eb_time, as.numeric(full_roc_eb$auc))
write.table(ddcor_line, "auc_results_classes.txt", sep = "\t", quote = FALSE, append = TRUE, col.names = FALSE,
	row.names = FALSE)
...
if(compare_eb_classes){
	...
	tiff(file = paste0("ebcoexpress_classes_", corr_classes, n_samples, "_samples_seed", seed, ".tiff"),
			width = 4, height = 4, units = 'in', res = 300)
...
if(discordant_test){
	...
	print(paste0("time taken to run the discordant pipeline with ", corr_classes, n_samples, " samples..."))
	...
	ddcor_line = c("discordant", corr_classes, n_samples, seed, discordant_time, as.numeric(full_roc_discordant$auc))
	write.table(ddcor_line, "auc_results_classes.txt", sep = "\t", quote = FALSE, append = TRUE, col.names = FALSE,
	row.names = FALSE)
	...
	ddcor_line = c("discordant_t", corr_classes, n_samples, seed, discordant_time, as.numeric(full_roc_discordant_t$auc))
	write.table(ddcor_line, "auc_results_classes.txt", sep = "\t", quote = FALSE, append = TRUE, col.names = FALSE,
	row.names = FALSE)
	...
	if(compare_discordant_classes){
		...
		tiff(file = paste0("discordant_classes_", corr_classes, n_samples, "_samples.tiff"),
				width = 4, height = 4, units = 'in', res = 300)
...
#############################################
#plot comparing the methods 

if(compare_ddcor_eb_discordant){

	tiff(file = paste0("ddcor_vs_ebcoexpress_vs_discordant_", corr_classes, n_samples, "_samples_seed", seed, ".tiff"),
						width = 4, height = 4, units = 'in', res = 300)
